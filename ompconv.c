...
//            if (source->R[i]!=output->R[i] || source->G[i]!=output->G[i] || source->B[i]!=output->B[i]) printf("At position i=%d %d!=%d,%d!=%d,%d!=%d\n",i,source->R[i],output->R[i], source->G[i],output->G[i],source->B[i],output->B[i]);
        ...
        #pragma omp parallel sections num_threads(4)
        {
               #pragma omp section 
                {
                    convolve2D(source->R, output->R, source->ancho, (source->altura/partitions)+halosize, kern->Vern, kern->kernelX, kern->kernelY);
                }
...
// ./a.out im03.ppm '01. Edge.txt' cekibray.ppm 1
// Imatge: im03.ppm
// ISizeX : 800
// ISizeY : 600
// kSizeX : 3
// kSizeY : 3
// 0.274286 seconds elapsed for Reading image file.
// 0.011081 seconds elapsed for copying image structure.
// 0.000578 seconds elapsed for Reading kernel matrix.
// 0.153771 seconds elapsed for make the convolution.
// 0.304769 seconds elapsed for writing the resulting image.
// 0.744618 seconds elapsed